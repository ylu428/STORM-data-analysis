% Update v2: Some users use different light programs which have different
% start_frame and end_frame. This update allows user to assign both
% start_frame and end_frame. Function "process1" was also updated, with
% 1010 as default start_frame and 21009 as default end_frame. 
...
offset = 0;
start_frame = 1540; % GFP 0-9, pre-bleach 10-1009 frames
end_frame = 8539; % total 21110 frames
start_frame = start_frame + offset;
end_frame = end_frame + offset;



FWHMtest= startAutomation(start_frame, end_frame);
...
for i = 1:length(FWHMtest)
    if FWHMtest(i).pass_FWHMthr_ratio_corr~='1'
        ...
        if length(str2num(FWHMtest(i).pass_FWHMthr_par_num))>=2
            temp_tif = fullfile(FWHMtest(i).folder, FWHMtest(i).NP1_tif_file);
            temp_SML = fullfile(FWHMtest(i).folder, FWHMtest(i).SML_file);
            test_result = process1(FWHMtest(i).folder, temp_tif, temp_SML, str2num(FWHMtest(i).pass_FWHMthr_par_num), start_frame, end_frame);
...
function FWHMtest = startAutomation(start_frame, end_frame)
    ...
    for i = 1:length(file_list)
        ...
        test_result = process1(folder_name, tif_file, csv_file, select_NP, start_frame, end_frame);
