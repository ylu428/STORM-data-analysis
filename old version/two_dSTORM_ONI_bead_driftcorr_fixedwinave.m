...
%maxSMLs_1 = 3000; % remove SMLs from NP
%minSMLs_2 = 5;
runDBSCAN = false;
...
temp = split(raw_tif_filename, '_NP_f1.tif'); 
...
%%step1: load particle info
           
load(strcat(TopDir, matlaboutbase, parse_particle_filename,'.mat'));

%%step1: load NP low res image
Im_NP = imread(strcat(TopDir,char(raw_tif_filename)));
...
if exist(strcat(TopDir,char(temp(1)),'_NP_f1001.tiff'),"file")==2 % 2: A file with the name exist.
    Im_NP_after_prebleach = imread(strcat(TopDir,char(temp(1)),'_NP_f1001.tiff'));
else
    Im_NP_after_prebleach = imread(strcat(TopDir,char(temp(1)),'_NP_f1001.tif'));
end
peaksizepix = 5;
