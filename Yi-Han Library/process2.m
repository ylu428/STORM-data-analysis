function test_result = process2(folder_name, raw_tif_filename, SMLs_filename, select_NP )
    ...
    realnameTIF = dir(fullfile(folder_name, strcat('*', raw_tif_filename)));
    disp(realnameTIF);
    realnameCSV = dir(fullfile(folder_name, strcat('*', SMLs_filename)));
    disp(realnameCSV)
    ...
    [TopDir, name, ext] = fileparts(realnameTIF.name); % Extract the directory and filename separately
